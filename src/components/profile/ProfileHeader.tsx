      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            {name}
            {isArtist && isVerified && (
              <span className="bg-blue-500 text-xs px-2 py-1 rounded-full">تأیید شده</span>
            )}
          </h1>
          <p className="text-gray-400">{isArtist ? 'هنرمند' : 'کاربر عادی'}</p>
        </div>
      </div>
      <div>
        <span className={`px-4 py-2 rounded-full font-bold ${subscriptionPlan === 'Premium' ? 'bg-green-500 text-black' : 'bg-gray-700 text-white'}`}>
          {subscriptionPlan === 'Premium' ? 'پرمیوم' : 'رایگان'}
        </span>
      </div>
    </div>
  );
